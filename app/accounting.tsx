...
function EditorModal({ visible, title, children, colors, onClose, onSave }: { visible: boolean; title: string; children: React.ReactNode; colors: ThemeColors; onClose: () => void; onSave: () => void }) {
  ...
  return <Modal visible={visible} animationType={Platform.OS === 'ios' ? 'slide' : 'fade'} transparent onRequestClose={dismissAndClose}><KeyboardAvoidingView style={styles.modalBackdrop} behavior="padding">{content}</KeyboardAvoidingView></Modal>;
}
